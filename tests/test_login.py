...
def test_correct_login(login_page: LoginPage):
    event_page = login_page.login_with_responsible()
    event_page.assert_profile_button_visible()


def test_incorrect_login(login_page: LoginPage):
    fake_username = login_page.get_fake_username()
...
def test_incorrect_password(login_page: LoginPage):
    correct_username = login_page.get_responsible_username()
    fake_password = login_page.get_fake_password()
    login_page.do_login(correct_username, fake_password)
    login_page.assert_login_error_visible()


def test_login_button_disabled_without_username(login_page: LoginPage):
    login_page.enter_password("test_password")
    login_page.assert_login_button_disabled()


def test_login_button_disabled_without_password(login_page: LoginPage):
    login_page.enter_username("test_username")
    login_page.assert_login_button_disabled()


def test_click_krtech_logo_from_login_page(login_page: LoginPage):
    krtech_page = login_page.click_krtech_logo()
    login_page.assert_krtech_website_opened(krtech_page)


def test_click_telegram_button_from_login_page(login_page: LoginPage):
    telegram_page = login_page.click_telegram_button()
    login_page.assert_telegram_website_opened(telegram_page)


def test_click_vkontakte_button_from_login_page(login_page: LoginPage):
