...
def test_create_document_with_only_required_fields(main_page_with_responsible):
    doc_create_page = main_page_with_responsible.open_document_creation_page('Входящий (Автотест)')
...
def test_create_document_via_bottom_edit_button(main_page_with_responsible):
    doc_create_page = main_page_with_responsible.open_document_creation_page('Внутренний. Без Шаблона Печати (Автотест)')
    doc_create_page.fill_short_description()
